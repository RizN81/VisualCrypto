...
public class Crypting {
	...
	public static BufferedImage loadAndCheckSource(File sourceFile, int width, int height, boolean resize) {
		...
		for (int i = 0; i < imgSrc.getHeight(); i++)
		{
			for (int j = 0; j < imgSrc.getWidth(); j++)
			{
				...
				//the value will be -5461077 or similiar negative value
				// check if pixel is either fully transparent or black
	...
	public static BufferedImage encryptImage(BufferedImage imgKey, BufferedImage imgSrc) {
		...
		if (imgSrc.getWidth() != imgKey.getWidth() / 2 || imgSrc.getHeight() != imgKey.getHeight() / 2)
			return null;
			...
			for (int x = 0; x < imgEncr.getWidth(); x += 2)
			{
				...
				else
				{
					...
					if (imgKey.getRGB(x, y) == Color.BLACK.getRGB())
						encrGraphics.fillRect(x, y, 1, 1);
					if (imgKey.getRGB(x + 1, y) == Color.BLACK.getRGB())//<-- -16777216
						encrGraphics.fillRect(x + 1, y, 1, 1);
					if (imgKey.getRGB(x, y + 1) == Color.BLACK.getRGB())
						encrGraphics.fillRect(x, y + 1, 1, 1);
					if (imgKey.getRGB(x + 1, y + 1) == Color.BLACK.getRGB())//<-- -16777216
						encrGraphics.fillRect(x + 1, y + 1, 1, 1);
				}
			}
		}
	...
	public static BufferedImage decryptImage(BufferedImage imgOverlay) {
		...
		for (int yOver = 0, yCln = 0; yOver < imgOverlay.getHeight(); yOver += 2, ++yCln)
		{
			...
			{
				if (imgOverlay.getRGB(xOver, yOver) == Color.BLACK.getRGB() && imgOverlay.getRGB(xOver + 1, yOver) == Color.BLACK.getRGB()
						&& imgOverlay.getRGB(xOver, yOver + 1) == Color.BLACK.getRGB() && imgOverlay.getRGB(xOver + 1, yOver + 1) == Color.BLACK.getRGB())
				{
